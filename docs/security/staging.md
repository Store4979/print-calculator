...
| stage 0 (staging half) — verified deployment context | **DEPLOYED to staging 2026-09-23** (`6c47eb6`, deploy `6ab3be3c…`): probe 0a PASS (`source: LAMBDA_TASK_ROOT`, `context: production`, `flagPresent: true`), 0c PASS (four 401s); 0b-prod PASS as evidence (b) + flag-absence half of (d) (`context: deploy-preview`, `flagPresent: false`, no service-role key); 0b-staging MISSING — the staging site built no preview for PR #48. Retained key-holding permalinks inventoried: `docs/security/deploy-inventory.md`. `release2Allowed()` gains a third condition read from `netlify/lib/deploy-context.json`, written at build; the production-ref refusal stays. **Under `netlify dev` and any local build the writer records nulls (or `dev`), the reader rejects them, and every Release 2 endpoint answers 404 — by design**; local work that needs them sets `RELEASE2_CONTEXTS=production,dev` deliberately. evidence (c) of §4.1 recorded | — |
